 ...
 * Le joueur est marqué déconnecté ; si trop peu de joueurs restent, la partie est close.
...
export async function handleGameDisconnect(io: TypedServer, socket: TypedSocket): Promise<boolean> {
  ...
  setRoomPlayerConnected(room, socket.id, false);
  room.game.setPlayerConnected(socket.id, false);
